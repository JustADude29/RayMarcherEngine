...
int main(int argv, char *argc[]) {
  ...
  window.create(sf::VideoMode(WIDTH, HEIGHT), "RayMarching");
  window.setFramerateLimit(curr_fps);
  window.setActive(true);

  // loading shader
  sf::Shader ShaderFrag[5];
  if (!ShaderFrag[0].loadFromFile(curr_path + "rayMarch.frag",
                                  sf::Shader::Fragment)) {
  ...
  overlays::button exit_button(
      "EXIT", font, sf::Color::White,
      sf::Vector2f(7 * resolution.x / 8, 9.5f * resolution.y / 10), 40);

  sf::Image increaseImage;
  sf::Image increaseImageDark;
  overlays::button increaseSteps_button(
      "", font, sf::Color::White,
      sf::Vector2f(7.5f * resolution.x / 8, resolution.y / 10), 10, true);
  if (increaseImage.loadFromFile(curr_path + "assets/plus.png") &&
      increaseImageDark.loadFromFile(curr_path + "assets/plus_dark.png")) {
  ...
  overlays::button decreaseSteps_button(
      "", font, sf::Color::White,
      sf::Vector2f(6.5f * resolution.x / 8, resolution.y / 10), 10, true);
  if (decreaseImage.loadFromFile(curr_path + "assets/minus.png") &&
      decreaseImageDark.loadFromFile(curr_path + "assets/minus_dark.png")) {
  ...
  overlays::button stepsTextButton(
      "1000", font, sf::Color::Black,
      sf::Vector2f(7 * resolution.x / 8, resolution.y / 10), 40);
  sf::Text stepsInstructionsText(
      "Current number of Ray Marching Steps(click to reset)", font, 15);
  stepsInstructionsText.setFillColor(sf::Color(100, 20, 20, 120));
  stepsInstructionsText.setPosition(
      sf::Vector2f(6.3f * resolution.x / 8, 0.5f * resolution.y / 10));

  sf::Color defCol1 = sf::Color::Black;
  overlays::button infiniterButton(
      "Click to toggle infinite", font, defCol1,
      sf::Vector2f(7 * resolution.x / 8, 2 * resolution.y / 10), 20);

  sf::Color defCol2 = sf::Color::Black;
  overlays::button outlineButton(
      "Click to toggle outline", font, defCol2,
      sf::Vector2f(7 * resolution.x / 8, 2.5 * resolution.y / 10), 20);

  // UI menu background
  sf::RectangleShape rect(
      sf::Vector2f(resolution.x / 4 - 20.0f, resolution.y - 10.0f));
  rect.setOrigin(rect.getGlobalBounds().width / 2,
  ...
  while (running) {
    ...
    // WASD movement
    Eigen::Vector2f angle = Eigen::Vector2f(mouseDelta.y - resolution.y * 0.5,
                                            mouseDelta.x - resolution.x * 0.5);
    ...
    Eigen::Vector3f forward =
        getCamRot(angle * mouseSens) * Eigen::Vector3f(0, 0, 0.01) * speedMult;
    Eigen::Vector3f right =
        getCamRot(angle * mouseSens) * Eigen::Vector3f(0.01, 0, 0) * speedMult;
    Eigen::Vector3f up =
        getCamRot(angle * mouseSens) * Eigen::Vector3f(0, 0.01, 0) * speedMult;
    ...
    if (mouseEnabled) {
      if (leftClickListen()) {
        if (window.mapPixelToCoords(sf::Mouse::getPosition(window)).x <
            3 * resolution.x / 4) {
          sf::Mouse::setPosition(center, window);
          window.setMouseCursorVisible(false);
          window.setMouseCursorGrabbed(true);
    ...
    exit_button.update(event, window, sf::Color::White,
                       sf::Color(25, 23, 20, 180), "EXIT");
    increaseSteps_button.update(event, window, sf::Color::White,
                                sf::Color::White, "");
    decreaseSteps_button.update(event, window, sf::Color::White,
                                sf::Color::White, "");
    stepsTextButton.update(event, window, sf::Color(3, 3, 3, 255),
                           sf::Color::Blue, std::to_string(steps));
...
Eigen::Matrix3f getCamRot(Eigen::Vector2f angle) {
  Eigen::Vector2f cos =
      Eigen::Vector2f(std::cos(angle.x()), std::cos(angle.y()));
  Eigen::Vector2f sin =
      Eigen::Vector2f(std::sin(angle.x()), std::sin(angle.y()));

  float vals[9] = {cos.y(),           0,
                   -1 * sin.y(),      sin.y() * sin.x(),
                   cos.x(),           cos.y() * sin.x(),
                   sin.y() * cos.x(), -1 * sin.x(),
                   cos.y() * cos.x()};
